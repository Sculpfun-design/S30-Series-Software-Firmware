...
void report_feedback_message(Message message) {  // ok to send to all clients
    ...
    {
         {   
#ifdef COMPATIBLE_WITH_LIGHTBURN 
        if(message == Message::AlarmLock)
        {
            auto it = MessageText.find(message);
            grbl_msg_sendf(CLIENT_SERIAL, MsgLevel::None, it->second);
        }else
        {
#endif // end COMPATIBLE_WITH_LIGHTBURN
            auto it = MessageText.find(message);
...
#ifndef COMPATIBLE_WITH_LIGHTBURN 
...
#endif
#else // else COMPATIBLE_WITH_LIGHTBURN
    grbl_sendf(client, "[VER:%s.%s:]\r\n[OPT:V,15,128]\r\n", GRBL_VERSION, GRBL_VERSION_BUILD);
#endif // end COMPATIBLE_WITH_LIGHTBURN
}
